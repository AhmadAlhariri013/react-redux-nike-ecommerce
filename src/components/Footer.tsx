...
const Footer = ({ footerAPI: { titles, links } }: IFooterProps) => {
  const [Year, setYear] = useState<number>();
  useEffect(() => {
    const getYear = () => setYear(new Date().getFullYear() as number);
    getYear();
  }, []);
      ...
      <div className="nike-container text-slate-200">
        ...
        <div className="mt-5 text-center">
          <p className="text-sm md:text-center">
            ...
            Reserved Rights{" "}
            <span className="font-semibold">Ahmad Alhariri {Year}</span>
          </p>
        </div>
      </div>
    </footer>
